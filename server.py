...
def run_pipeline(job_id: str, url: str, params: dict, local_mp3_path: Optional[str] = None):
    ...
    try:
        ...
        # YouTube upload
        job["youtube_urls"] = []
        if YOUTUBE_REFRESH_TOKEN and OPENAI_API_KEY and job["suno_tracks"]:
            thumbnail_url = (job.get("video_info") or {}).get("thumbnail_url", "")
            if thumbnail_url:
                try:
                    log("Editing album art...")
                    edited_cover_path = edit_album_art(thumbnail_url, OPENAI_API_KEY)
                    log(f"Album art edited: {edited_cover_path}")

                    for i, track in enumerate(job["suno_tracks"], 1):
                        try:
                            log(f"Uploading track {i} to YouTube...")
                            ...
                            job["youtube_urls"].append(yt_url)
                            track["youtube_url"] = yt_url
                            log(f"Track {i} uploaded: {yt_url}")
                        except Exception as e:
                            log(f"YouTube upload failed for track {i}: {e}")
                except Exception as e:
                    log(f"Image editing or YouTube upload failed: {e}")
