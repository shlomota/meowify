...
YOUTUBE_REFRESH_TOKEN = _ENV.get("YOUTUBE_REFRESH_TOKEN", "")
...
def _upload_to_youtube_bg(job_id: str, track_idx: int):
    ...
    try:
        # Initialize status
        if "youtube_upload_status" not in job:
            job["youtube_upload_status"] = {}
        job["youtube_upload_status"][track_idx] = {"status": "downloading", "message": "Downloading album art..."}
        save_job(job_id, job)
            ...
            for quality in ["maxresdefault", "sddefault", "hqdefault", "mqdefault"]:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        job["youtube_upload_status"][track_idx] = {"status": "editing", "message": f"Editing album art ({quality})..."}
                        save_job(job_id, job)
        ...
        # Upload video
        job["youtube_upload_status"][track_idx] = {"status": "uploading", "message": "Uploading to YouTube..."}
        save_job(job_id, job)
        ...
        yt_url = upload_to_youtube(
            track["path"],
            title=track_title,
            description=f"🎵 Meowify Cover\n\n{info.get('title', 'Unknown')}\n\nMusic generated by Suno AI",
            refresh_token=YOUTUBE_REFRESH_TOKEN,
            thumbnail_url=edited_cover_path,
            tags=["meow", "cover", "suno"]
        )

        # Success
        track["youtube_url"] = yt_url
        job["youtube_upload_status"][track_idx] = {"status": "done", "message": "✓ Uploaded to YouTube"}
        save_job(job_id, job)
        ...
        job["youtube_upload_status"][track_idx] = {"status": "error", "message": f"Error: {str(e)[:80]}"}
        save_job(job_id, job)
        if edited_cover_path and os.path.exists(edited_cover_path):
            try:
                os.remove(edited_cover_path)
...
@app.post("/job/{job_id}/upload-youtube")
async def upload_youtube(job_id: str, request: Request, track_idx: int = Form(0)):
    user = current_user(request)
    if not user:
        return RedirectResponse("/login", status_code=303)
    job = JOBS.get(job_id) or load_job(job_id)
    if not job or not can_access_job(user, job):
        raise HTTPException(403)
    if not job.get("suno_tracks") or track_idx >= len(job["suno_tracks"]):
        raise HTTPException(400, "Invalid track index")
    if not YOUTUBE_REFRESH_TOKEN or not OPENAI_API_KEY:
        raise HTTPException(500, "YouTube upload not configured")

    # Start background upload
    threading.Thread(
        target=_upload_to_youtube_bg,
        args=(job_id, track_idx),
        daemon=True,
    ).start()
...
@app.get("/job/{job_id}/upload-youtube/status")
async def upload_youtube_status(job_id: str, request: Request, track_idx: int = 0):
    user = current_user(request)
    if not user:
    ...
    status_info = (job.get("youtube_upload_status") or {}).get(track_idx, {})
    status = status_info.get("status", "idle")
    message = status_info.get("message", "")
    yt_url = job["suno_tracks"][track_idx].get("youtube_url") if track_idx < len(job.get("suno_tracks", [])) else None
    ...
    if yt_url:
        ...
        html = f'<small class="text-danger">{message}</small>'
    else:
        # Uploading
        html = f'<small class="text-info"><span class="spinner-border spinner-border-sm me-2" style="width:12px;height:12px"></span>{message}</small>'
        if status != "done":
            # Keep polling if not done
            html += '''<script>
            setTimeout(() => {
                htmx.ajax('GET', window.location.href.split('?')[0] + '?track_idx=%d', '#yt-status-%d')
            }, 1000);
            </script>''' % (track_idx, track_idx)
