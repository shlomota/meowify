...
def detect_chorus(audio_path: str, default_start: float = 45.0, duration: float = 30.0) -> tuple[float, str]:
    ...
    for s in range(30, min(91, int(total - duration))):
        if s + w >= len(rms):
            break
...
def apply_masking(src: str, dst: str) -> str:
    y, sr = sf.read(src)
    ...
    drive = 1.4
    y = 0.85 * y + 0.15 * (np.tanh(y * drive) / np.tanh(drive))
    rms = np.sqrt(np.mean(y**2))
    y = y + np.random.normal(0, rms * 10**(-35/20), y.shape).astype(np.float32)
...
st.title("🐱 Meowify")
st.caption("Create a meow cover of your favourite song")
...
with st.sidebar:
    ...
    st.subheader("Chorus")
    chorus_start = st.number_input("Default start (s)", value=45.0, step=5.0)
...
if run and url:
    ...
    if kie_key and aws_id and aws_sec:
        ...
        with st.status("Uploading to S3...", expanded=True) as status:
            try:
                upload_src = meow_local
                s3_key = f"meowify/{base}_meowified.wav"
                presigned_url = s3_upload_presign(upload_src, s3_bucket, s3_key, aws_id, aws_sec)
                log(f"Uploaded meowified WAV: s3://{s3_bucket}/{s3_key} (presigned URL valid 2h)")
                st.write(f"Uploaded `{s3_key}`")
                status.update(label="Uploaded to S3 ✓", state="complete")
            except Exception as e:
                log(f"ERROR: {e}")
        ...
        with st.status("Generating Suno cover...", expanded=True) as status:
            try:
                ...
                result = suno_submit(
                    presigned_url, kie_key, meow_count, suno_style, cover_title, suno_model,
                ...
                suno_payload = result["payload"]
                st.session_state.files["suno_payload"] = suno_payload
                log(f"Suno task submitted: {task_id}")
                ...
                while True:
                    ...
                    suno_data = r.json()["data"]
                    suno_status = suno_data["status"]
                    elapsed = time.time() - t0
                    poll_slot.write(f"Status: **{suno_status}** — {elapsed:.0f}s elapsed")
                    log(f"Suno poll: {suno_status} ({elapsed:.0f}s)")
                    if suno_status in terminal:
                        break
                    if elapsed > 600:
                        raise TimeoutError("Suno timed out after 10 min")
                    time.sleep(10)

                tracks = suno_data.get("response", {}).get("sunoData", [])
                log(f"Suno complete: {len(tracks)} tracks, status={suno_status}")
                ...
                for i, track in enumerate(tracks, 1):
                    ...
                    st.session_state.suno_tracks.append({
                        "path": out,
